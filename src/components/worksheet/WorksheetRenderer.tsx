            ...
            <>
                {parts.map((part, i) => {
                    const match = patterns.find(
                        (p) => p.text.toLowerCase() === part.toLowerCase(),
                    );
                    if (match) {
                        return (
                            <StudyContextTrigger
    ...
    return (
        ...
        >
            ...
            <div className="relative mb-10 border-b-[3px] border-double border-[var(--ws-gold)] pb-8 pt-12 text-center">
                <span className="absolute left-0 top-[50px] text-2xl text-[var(--ws-gold)] print:hidden">
                    ☯
                </span>
                <span className="absolute right-0 top-[50px] text-2xl text-[var(--ws-gold)] print:hidden">
                    ☯
                </span>
            ...
            <div className="mb-10 flex border-[1.5px] border-[var(--ws-gold)] bg-[#f5edd8] print:bg-transparent">
                {["Name", "Date", "Class", "Score"].map((label) => (
                    <div
                        key={label}
                        className="flex-1 border-r border-[var(--ws-gold)]/30 p-2.5 last:border-r-0"
                    >
            ...
            <section className="mb-11">
                <div className="font-mono text-[0.7rem] uppercase tracking-[0.2em] text-[var(--ws-red)]">
                    Part 1
                </div>
                    ...
                    {worksheet.vocabulary.map((item, i) => (
                        ...
                        >
                            ...
                            <div className="mt-1.5 flex items-center gap-2">
                                <span className="text-[10px] font-bold uppercase text-[var(--ws-gold)] print:hidden">
                                    Meaning:
                                </span>
                                <span
                                    className={cn(
                                        ...
                                        !revealedVocab.has(i) &&
                                            "blur-[4px] group-hover:blur-0",
                                    )}
                                >
                                    {item.koreanMeaning}
                                </span>
                            </div>
                            <em className="mt-2 block text-[0.85rem] italic leading-tight text-[var(--ws-faded)]">
                                {item.example}
                            </em>
                        </div>
                    ))}
            ...
            <section className="break-before-page">
                ...
                <div className="mt-5 space-y-3.5">
                    {Array.from({ length: 12 }).map((_, i) => (
                        <div
                            key={i}
                            className="min-h-[28px] border-b border-[#c0b090]"
                        />
                    ))}
                </div>
            </section>
                ...
                <div className={showAnswer ? "block" : "hidden print:block"}>
                    <div className="border-t-[3px] border-double border-[var(--ws-gold)] pt-8">
                        ...
                        <div className="mb-8 space-y-4">
                            <p className="mb-2 font-mono text-[0.65rem] uppercase tracking-widest text-[var(--ws-faded)]">
                                Multiple Choice
                            </p>
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                {worksheet.multipleChoice.map((q, i) => (
                                    <div
                                        key={i}
                                        className="border border-[var(--ws-gold)]/30 bg-[#f5edd8] p-4 text-[0.88rem] print:bg-transparent"
                                    >
                                        <p className="font-bold text-[var(--ws-red)]">
                                            Q{i + 1} → {q.answer}
                                        </p>
                                        <p className="mt-1 italic leading-relaxed text-[var(--ws-ink)]/80">
                                            {q.explanation}
                                        </p>
                                    </div>
                                ))}
                        ...
                        <div className="mb-8 space-y-4">
                            ...
                            <div className="space-y-4 text-[0.88rem] leading-relaxed">
                                {worksheet.shortAnswer.map((q, i) => (
                                    <div
                                        key={i}
                                        className="border-l-2 border-[var(--ws-gold)]/40 bg-[#f5edd8]/30 p-4 print:bg-transparent"
                                    >
                                        <p className="font-bold text-[var(--ws-deep)]">
                                            Q
                                            {worksheet.multipleChoice.length +
                                                i +
                                                1}{" "}
                                            Model Answer:
                                        </p>
                                        ...
                                        <p className="mt-2 italic text-[var(--ws-faded)]">
                                            {q.explanation}
                                        </p>
                                    </div>
                                ))}
                        ...
                        <div className="border-t border-[var(--ws-gold)]/20 pt-6">
                            <h4 className="mb-3 font-mono text-[0.7rem] uppercase text-[var(--ws-faded)]">
                                📝 Model Essay Example
                            </h4>
...
function StudyContextContent({
    ...
    const tip = splitText(
        type === "phrase"
            ? (item as Phrase).explanation
            : (item as SentenceAnalysis).tip,
    );

    return (
        <div className="flex flex-col gap-4 p-4 md:p-0">
            <div className="space-y-1">
                <p className="text-mono-label text-[var(--ws-gold)]">
                    {type === "phrase" ? "Key Phrase" : "Grammar Point"}
                </p>
                <h3 className="text-xl font-bold text-[var(--ws-deep)]">
                    {type === "phrase"
                        ? (item as Phrase).pattern
                        : (item as SentenceAnalysis).structureLabel}
                </h3>
            </div>

            <div className="rounded-lg bg-[var(--ws-gold)]/5 p-4 space-y-3 border border-[var(--ws-gold)]/10">
                <div className="space-y-1">
                    <p className="text-[10px] font-bold uppercase text-[var(--ws-faded)]">
                        Explanation
                    </p>
            ...
            {type === "phrase" && (
                <div className="space-y-2">
                    ...
                    <p className="text-[0.9rem] text-[var(--ws-faded)]">
                        💬 {(item as Phrase).dailyUse}
                    </p>
                </div>
            )}
            ...
            {type === "sentence" && (
                <div className="flex flex-wrap gap-1.5">
                    {(item as SentenceAnalysis).parse.map((chunk, i) => (
                        <div
                            key={i}
                            className={`rounded px-2 py-1 shadow-sm ${CSS_CLASS_COLORS[chunk.cssClass] || "bg-muted"}`}
                        >
                            <p className="text-[9px] font-mono uppercase opacity-70">
                                {chunk.role}
                            </p>
                ...
                <div className="mx-auto w-full max-w-lg overflow-y-auto px-4 pb-8 pt-2">
                    ...
                    <DrawerFooter className="px-0 mt-4">
                        <DrawerClose asChild>
                            <Button
                                variant="outline"
                                className="w-full border-[var(--ws-gold)]/30 text-[var(--ws-faded)]"
                            >
                                Close
                            </Button>
                        </DrawerClose>
