...
const STEPS = [
  ...
  },
  {
    ...
    bg: '#f5f3ff',
    description:
      'AI가 생성한 빈칸 채우기·객관식·주관식 문제를 푼다. PDF로 다운로드해 손으로 적어도 좋고, 화면에서 바로 풀어도 된다. 완료 후 필기 파일을 업로드하면 기록으로 남는다.',
...
export default function GuidePage() {
  return (
    <div className='container mx-auto max-w-2xl px-4 py-8'>
      ...
      {/* Hero */}
      <div className='mb-10'>
        <p className='text-mono-label text-muted-foreground mb-2'>LEARNING METHOD</p>
        <h1 className='text-[2rem] font-semibold leading-tight tracking-tight mb-3'>
          TED-fi 4단계 학습법
        </h1>
        <p className='text-muted-foreground text-sm leading-relaxed'>
          매일 TED-Ed 영상 한 편을 4단계로 깊이 학습한다.
          단순 반복이 아닌 <strong className='text-foreground'>능동적 처리</strong>를 통해 표현이 자연스럽게 내재화된다.
        </p>
        <a
            ...
            <div key={step.n} className='flex gap-4'>
              ...
              >
                ...
                <div
                  ...
                  style={{ backgroundColor: step.color + '18', color: step.color === '#010120' ? '#3d3d6e' : step.color }}
                >
      ...
      {/* Flow summary */}
      <div className='mb-10'>
        <h2 className='text-base font-semibold mb-4'>한눈에 보는 학습 흐름</h2>
        <div className='grid grid-cols-4 gap-2'>
          {STEPS.map((step, i) => (
            <div key={step.n} className='flex flex-col items-center gap-1.5 relative'>
              <div
              ...
              <span className='text-xs text-center font-medium leading-tight'>{step.label}</span>
      ...
      <div className='rounded-xl border border-border p-4 bg-muted/40'>
        <p className='text-xs text-muted-foreground leading-relaxed'>
          ...
          의 방법론을 바탕으로 설계되었으며, AI 생성 학습자료와 결합해 TED-fi에 맞게 재구성한 것이다.
        </p>
      </div>
