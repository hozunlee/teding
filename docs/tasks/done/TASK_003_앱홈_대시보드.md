...
- [x] SSR 환경에서 `/api/today` 정보를 패치하여 오늘의 영상 썸네일과 정보를 렌더링하는가
- [x] 현재 로그인한 유저의 스트릭(Streak) 정보를 바탕으로 이번 주 스탬프를 UI에 노출하는가
- [x] "학습 시작" 버튼 클릭 시 정확히 `/study?step=1` (혹은 기존 진행된 스텝)으로 이동하는가
