# TED-fi — Product Requirements Document
...
TED-fi는 TED-Ed 영상 한 편을 4단계로 학습하는 AI 기반 영어 학습 앱이다.  
...
async function uploadAnnotated(file: File, userId: string, videoId: string) {
  ...
  await fetch('/api/upload', {
    method: 'POST',
    body: JSON.stringify({ userId, videoId, fileUrl: data.path, fileName: file.name }),
  });
}
```
...
Kakao.Share.sendDefault({
  ...
  content: {
    ...
    link: { mobileWebUrl: 'https://tedfi.app', webUrl: 'https://tedfi.app' },
  },
  buttons: [{ title: '나도 학습하기', link: { mobileWebUrl: 'https://tedfi.app' } }],
});
```
...
if (navigator.share) {
  navigator.share({ title: 'TED-fi', text: shareText });
} else {
